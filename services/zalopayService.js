...
class ZaloPayService {
  constructor() {
    ...
    // Sử dụng endpoint sandbox v2 đúng
    this.endpoint =
  ...
  async createPayment(orderId, amount, description, embedData = {}) {
    try {
      ...
      const appTransId = `${yymmdd}_${transID}`;
      ...
      if (response.data.return_code === 1 || response.data.returncode === 1) {
        return {
          ...
          zpTransToken:
            response.data.zp_trans_token || response.data.zptranstoken,
          appTransId: appTransId,
        };
      } else {
        const returnCode =
          response.data.return_code || response.data.returncode;
        const returnMsg =
